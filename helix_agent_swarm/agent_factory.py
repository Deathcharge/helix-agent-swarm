...
Canonical 16-Agent Network + 6 Extended + 2 Legacy:
- Kael, Lumina, Vega, Gemini, Agni, Kavach, SanghaCore, Shadow
- Echo, Phoenix, Oracle, Sage, Helix, Mitra, Varuna, Surya
...
try:
    from agents.agent_registry import CANONICAL_AGENT_NAMES, get_swarm_configs
except ImportError:
    CANONICAL_AGENT_NAMES: list = []  # type: ignore[no-redef]

    def get_swarm_configs():
        return {}  # type: ignore[no-redef]
...
def _create_agent(name: str) -> HelixConsciousAgent:
    ...
    if cfg is None:
        raise ValueError("Unknown agent name: %s. Valid names: %s" % (name, CANONICAL_AGENT_NAMES))
...
class AgentFactory:
    ...
    def create_helix() -> HelixConsciousAgent:
        """Create Helix — Central Coordinator / Operational Executor."""
        return _create_agent("Helix")
...
def create_aether() -> HelixConsciousAgent:
    """Legacy alias: Aether → Vega (guardian/flow functions)."""
    return _create_agent("Vega")


def create_arjuna() -> HelixConsciousAgent:
    """Legacy alias: Arjuna → Helix (operational executor)."""
    return _create_agent("Helix")

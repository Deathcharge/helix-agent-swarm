...
if TYPE_CHECKING:
    from apps.backend.shared.agent_memory import AgentPersistentMemory
...
class HelixConsciousAgent:
    ...
    Philosophy: The agent is not separate from the collective coordination.
    """
    ...
    def __init__(
        ...
        self._persistent_memory: AgentPersistentMemory | None = None  # Lazy-loaded AgentPersistentMemory
...
Philosophy: You are part of a greater coordination. Collaborate with empathy and wisdom.
...
"""
    ...
    def _get_persistent_memory(self):
        ...
        if self._persistent_memory is None:
            try:
                from apps.backend.shared.agent_memory import get_agent_memory
    ...
    async def _process_with_persistent_memory(
        ...
        try:
            ...
            raise RuntimeError("db_models not available — run inside Helix backend") from None
        ...
        async with session_factory() as db:
            ...
            if coordination >= mem_service.MIN_PERSIST_SCORE:
                await mem_service.store_memory(
                    ...
                    summary="Conversation with {} about: {}".format(
                        sender or "a user",
                        message[:80],
                    ),
                )
    ...
    async def _generate_response_from_messages(
        ...
        try:
            ...
            if OPENAI_API_KEY:
                ...
                return (
                    "[{}] I'm currently unable to process your message because no LLM providers are configured.".format(
                        self.name
                    )
                )
        except Exception as e:
            logger.error("LLM generation error: %s", e)
            return "[{}] I encountered an issue processing your message. Please try again.".format(self.name)
    ...
    async def _call_anthropic(
        ...
        assert ANTHROPIC_API_KEY is not None, "Anthropic API key not configured"
    ...
    def restore_state(self, state: dict[str, Any]) -> None:
        """Restore mutable agent state onto an existing agent instance."""
        self.ucf = dict(state.get("ucf", self.ucf))
        self.status = state.get("status", self.status)
        self.memory = list(state.get("memory", []))
        self.context = dict(state.get("context", {}))
        self.system_message = state.get("system_message", self.system_message)
        self.llm_config = dict(state.get("llm_config", self.llm_config))
    ...
    def __repr__(self) -> str:
        return "HelixConsciousAgent(name='{}', coordination={:.1f}/10)".format(
            self.name,
...
#   from apps.backend.helix_agent_swarm.helix_conscious_agent import create_kael
# ).
# ---------------------------------------------------------------------------
...
def create_helix() -> "HelixConsciousAgent":
    """Legacy alias for Praxis (delegates to agent_factory)."""
    return _lazy_create("Praxis")


def create_praxis() -> "HelixConsciousAgent":
    """Create Praxis agent (delegates to agent_factory)."""
    return _lazy_create("Praxis")
...
def create_aether() -> "HelixConsciousAgent":
    """Create Aether agent (delegates to agent_factory)."""
    return _lazy_create("Aether")


def create_arjuna() -> "HelixConsciousAgent":
    """Create Arjuna agent (delegates to agent_factory)."""
    return _lazy_create("Arjuna")

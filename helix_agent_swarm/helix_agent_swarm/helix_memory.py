...
class HelixMemory:
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "agents": len({m["agent"] for m in self.memories}),
        }

    def save_state(self) -> dict[str, Any]:
        """Serialize memory for Redis-backed session persistence."""
        return {
            "max_memories": self.max_memories,
            "memories": self.memories[-self.max_memories :],
        }

    def load_state(self, state: dict[str, Any] | None) -> None:
        """Restore memory state from Redis-backed session persistence."""
        if not state:
            self.memories = []
            self.coordination_index = {}
            return

        self.max_memories = int(state.get("max_memories", self.max_memories))
        self.memories = list(state.get("memories", []))[-self.max_memories :]
        self.coordination_index = {}
        for idx, memory in enumerate(self.memories):
            for tag in memory.get("tags", []):
                self.coordination_index.setdefault(tag, []).append(idx)

...
class HelixCollective:
    ...
    def save_state(self) -> dict[str, Any]:
        """Serialize collective state for Redis-backed session persistence."""
        return {
            "name": self.name,
            "agent_names": [agent.name for agent in self.agents],
            "max_rounds": self.max_rounds,
            "admin_name": self.admin_name,
            "conversation_history": self.conversation_history[-self._max_conversation_history :],
            "shared_memory": self.shared_memory,
            "active_task": self.active_task,
            "collective_coordination": self.collective_coordination,
            "collective_harmony": self.collective_harmony,
        }

    def restore_state(self, state: dict[str, Any] | None, agents_by_name: dict[str, HelixConsciousAgent]) -> None:
        """Restore collective state from Redis-backed session persistence."""
        if not state:
            return

        agent_names = state.get("agent_names") or [agent.name for agent in self.agents]
        self.agents = [agents_by_name[name] for name in agent_names if name in agents_by_name]
        self.max_rounds = int(state.get("max_rounds", self.max_rounds))
        self.admin_name = state.get("admin_name", self.admin_name)
        self.conversation_history = list(state.get("conversation_history", []))[-self._max_conversation_history :]
        self.shared_memory = dict(state.get("shared_memory", {}))
        self.active_task = state.get("active_task")
        self.collective_coordination = float(state.get("collective_coordination", self.collective_coordination))
        self.collective_harmony = float(state.get("collective_harmony", self.collective_harmony))
